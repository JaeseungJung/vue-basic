...
export default {
  ...
  methods: {
    async getList() {
      this.list = await this.$api("/api/getList", "get", {}); // http://localhost
      console.log(this.list);
    },
    async createPerson() {
      const r = await this.$api("/api/createPerson2", "post", {
        param: [
          {
    ...
    async deletePerson(id) {
      const r = await this.$api("/api/deletePerson2", "post", {
        param: [id],
      });
      console.log(r);
      this.getList;
    ...
    async updatePerson(idx) {
      const r = await this.$api("/api/updatePerson2", "post", {
        param: [
          {
            first_name: this.list[idx].first_name,
            last_name: this.list[idx].lastName,
